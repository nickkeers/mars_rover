defmodule MarsRover.FileParser do
  ...
  defparsec(
    :robotfile,
    parse_header |> ignore(string("\n")) |> concat(times(parse_robot, min: 1) |> wrap() ) |> label("robot") |> eos()
  )
  end
  ...
  @spec parse(binary) ::
          ...
          | {:ok, %MarsRover.FileParser.RobotsFile{grid_size: {integer(), integer()}, robots: [%Robot{}]}}
